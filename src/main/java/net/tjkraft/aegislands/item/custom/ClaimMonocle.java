...
public class ClaimMonocle extends Item {
        ...
        for (int y = level.getMinBuildHeight(); y < level.getMaxBuildHeight(); y++) {
            for (int x = 0; x < 16; x++) {
                for (int z = 0; z < 16; z++) {
                    ...
                    if (level.getBlockEntity(check) instanceof ClaimAnchorBE anchor) {

                        if (anchor.getClaimTime() == 0) {
                            player.displayClientMessage(Component.translatable("msg.aegis_lands.no_time"), true);
                        } else {
    ...
    private void showClaimBox(ServerLevel level, BlockPos anchorPos) {
        ...
        int minY = anchorPos.getY() - ALServerConfig.MIN_ANCHOR_Y.get();
        int maxY = anchorPos.getY() + ALServerConfig.MAX_ANCHOR_Y.get();
